...
def captured(monkeypatch):
    ...
    seen = []
    monkeypatch.setattr(
        events.DEFAULT, "sink", lambda event, fields: seen.append({"event": event, **fields})
    )
    return seen
...
def test_auth_failed_on_a_revoked_token_names_it(captured, store):
    ...
    assert captured[0]["reason"] == "revoked"


def test_auth_failed_on_an_expired_token_says_expired(captured, store):
    raw, handle = store.mint(
        "agent", "scout", "cli:leon@dev", expires_at="2000-01-01T00:00:00+00:00"
    )
    authenticate(store, f"Bearer {raw}")
    assert captured[0]["reason"] == "expired"
    assert captured[0]["handle"] == handle
...
def test_first_use_is_not_emitted_twice_when_another_thread_claimed_it(captured, store):
    # Two concurrent requests both read last_used_at IS NULL. The in-lock claim
    # (last_touch stamped) must stop the second from emitting a duplicate.
    raw, _ = store.mint("agent", "scout", "cli:leon@dev")
    store.last_touch[token_hash(raw)] = time.monotonic()
    authenticate(store, f"Bearer {raw}")
    assert captured == []


def test_a_second_emitter_does_not_touch_the_default(captured, store):
    other = []
    custom = events.Emitter(sink=lambda event, fields: other.append(event))
    custom.token_created("demo", "h", "l", "r", "a")
    assert other == ["token.created"]
    assert captured == [], "the process default must be untouched"


def test_the_default_sink_writes_json_with_the_service(capsys, monkeypatch):
    """Pins the default sink's contract.

    Every other test replaces the default emitter's sink, so a default sink that
    wrote nothing — or dropped the service — would be invisible to the whole
    suite. This is the one test that exercises it.
    """
    monkeypatch.setattr(events.DEFAULT, "sink", events.stdout_sink)
    events.DEFAULT.token_created("demo", "abc123", "scout", "agent", "cli:leon@dev")
