...
def import_entries(store: Store, entries, *, emitter: events.Emitter | None = None) -> ImportReport:
    ...
    for entry in entries or []:
        ...
        if len(forms) != 1:
            ...
            emitter.token_import_rejected(store.service, role, reason)
            continue
        try:
            validate_role(role)
        except ValueError as exc:
            report.rejected.append((role, str(exc)))
            emitter.token_import_rejected(store.service, role, str(exc))
            continue
        ...
        if bad:
            ...
            emitter.token_import_rejected(store.service, role, bad)
            continue
        ...
        if existing is None:
            try:
                ...
                emitter.token_imported(store.service, handle, label, role)
                report.imported += 1
                continue
